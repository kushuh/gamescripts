...
.EXAMPLE
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Read-Secret-Missions.ps1 -SaveDirectory "D:\Steam\userdata\ACCOUNT_ID\738540\remote" -ShowSavePath
.NOTES
...
try {
    ...
    if ($null -eq $section -or $section.Size -ne 0x1300) {
        Stop-ProgressCheck 'Missing or unsupported Scenario data.'
    }
    ...
    Write-ProgressError 'Could not read Secret Mission progress' $_
    exit 1
}
