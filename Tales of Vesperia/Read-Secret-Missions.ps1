...
.EXAMPLE
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Read-Secret-Missions.ps1
.EXAMPLE
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Read-Secret-Missions.ps1 -SaveDirectory "D:\Steam\userdata\ACCOUNT_ID\738540\remote"
.NOTES
...
try {
    ...
    if ($null -eq $section -or $section.Size -ne 0x1300) {
        throw 'Missing or unsupported Scenario data.'
    }
    # PC Definitive Edition: script operation 660 -> 0x1405224c0 ->
    # 0x14050a5f0 sets these flags. Result jump table 0x1405139d8 maps
    # them to named SECRETMISSION01..25 rows at 0x140a21868.
    # Save copies at 0x1405bd452..0x1405bd4f5 confirm bitmap offset 0x1100.
    $flags = @(4026,4027,4028,4029,4030,4031,4032,4052,4033,4034,4035,4036,
               4037,4038,4039,4040,4041,4042,4043,4053,4044,4045,4046,4047,4048)
    $bosses = @('Zagi - Zaphias','Goliath','Gattuso','Zagi - Ragou''s ship',
                'Dreaded Giant','Gigalarva','Barbos','Cursed Wanderer - Atherum',
                'Zagi - Nordopolica','Pteropus','Outbreaker','Belius','Tison and Nan',
                'Schwann','Zagi - Heracles','Baitojoh','Estellise','Yeager','Alexei',
                'Cursed Wanderer - final encounter','Gusios','Khroma','Flynn',
                'Zagi - Tarqaron','Duke - second form')
    $rows = @(for ($i = 0; $i -lt 25; $i++) {
        $flag = $flags[$i]
        $value = $data[$section.Offset + 0x1100 + [int][Math]::Floor($flag / 8)]
        $done = ($value -band (1 -shl ($flag % 8))) -ne 0
        [pscustomobject]@{
            Mission = $i + 1
            Status = $(if ($done) { 'Done' } else { 'Not done' })
            Boss = $bosses[$i]
            Done = $done
            Flag = $flag
        }
    ...
    Show-ProgressHeader 'Secret Missions' $snapshot
    $completed = @($rows | Where-Object Done).Count
    Write-Host ("Completed in this save: $completed / 25")
    Write-Host ''
    Show-ProgressTable -Rows $rows -Columns @('Mission', 'Status', 'Boss') `
        -RightAlign @('Mission') -StatusColumn 'Status' -IsComplete { param($row) $row.Done }
    Write-Host 'Shows this save''s completion flags, independent of unlocked Steam achievements.'
    Write-Host 'Save in game before checking. This command only reads your saves.'
    exit 0
} catch {
    Write-Host ('Could not read Secret Mission progress: ' + $_.Exception.Message) -ForegroundColor Red
    exit 1
}
